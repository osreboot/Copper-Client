...
public class BRenderWorld extends BRender<EWorld>{
	...
	public void render(Environment environment, float delta, FRenderChannel channel){
		if(channel == FRenderChannel.BASE_TERRAIN){
			...
			HvlCoord pMin = WorldUtil.toTileSpace(new HvlCoord(player.location)
					.subtract((Display.getWidth() / 2f) / SRender.SCALE_WORLD + 2f, (Display.getHeight() / 2f) / SRender.SCALE_WORLD + 2f));
			HvlCoord pMax = WorldUtil.toTileSpace(new HvlCoord(player.location)
			...
			for(int x = xMin; x <= xMax; x++){
				for(int y = yMin; y <= yMax; y++){
					if(parent.tiles[x][y] != null){
						hvlDraw(parent.tiles[x][y].polygon, parent.tiles[x][y].material.color);
					}
				}
			}
		}
...
//					if(tile != null){
//						if(tile.faceVert) hvlDraw(hvlLine(tile.vertices[1], tile.vertices[2], 0.1f), Color.red);
//						if(tile.faceEast) hvlDraw(hvlLine(tile.vertices[0], tile.vertices[1], 0.1f), Color.red);
//						if(tile.faceWest) hvlDraw(hvlLine(tile.vertices[2], tile.vertices[0], 0.1f), Color.red);
//					}
//				}
//			}
//		}
