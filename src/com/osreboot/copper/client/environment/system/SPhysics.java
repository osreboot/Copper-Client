...
public final class SPhysics {
	...
	public static void update(Environment environment, float delta){
		...
		for(EPlayer player : environment.getEntities(EPlayer.class)){
			...
			HvlCoord pMin = CTile.toTileSpace(new HvlCoord(player.location).subtract(CALC_RANGE, CALC_RANGE));
			HvlCoord pMax = CTile.toTileSpace(new HvlCoord(player.location).add(CALC_RANGE, CALC_RANGE));
